...
(defun cart-to-angle (coords res)
  ...
  (let ((x (elt coords 0))
        (y (elt coords 1))
        (z (+ (elt coords 2) least-positive-short-float)))
...
;;;         (ele (get-val (vbap-ele vbap)))
    ...
    (let* ((beta (- 180 gamma))
           (b (/ (sin (* +atorad+ spread))
                 (sin (* +atorad+ beta))))
           (a (/ (sin (* +atorad+ (- 180 spread beta)))
                 (sin (* +atorad+ beta)))))
      ;; (incudine.util:msg :warn "gamma: ~a, beta: ~a, a: ~a, b: ~a" gamma beta a b)
      ;; (incudine.util:msg :warn "vscartdir: ~a" vscartdir)
      ;; (incudine.util:msg :warn "spreadbase: ~a" spreadbase)
...
;;;      (incudine.util:msg :warn "newspreaddir: ~a" spreaddir)
      )))
...
;;;    (incudine.util:msg :warn "add: ~a" spreaddir)
    (map '()
         ...
         ls-set-data)
    ;; (incudine.util:msg :warn "g: ~a" g)
    ;; (incudine.util:msg :warn "ls: ~a" ls)
    (dotimes (i dim)
      (when (< (aref g i) -0.01)
        (setf gains-modified t)))
    ...
    (let* ((azi (get-val (vbap-azi vbap)))
      ...
      (when (> spread 0)
        (case (vbap-data-dimen vbap-data)
          (3
           (setf (aref (aref spreadbase 0) 0) 0.0)
           (setf (aref (aref spreadbase 0) 1) 1.0)
           (setf (aref (aref spreadbase 0) 2) 0.0)
