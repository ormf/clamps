...
player-type and supplying them to make-instance of the poolevt."
  (let ((start-time (now))) ;; capture the value of time just to be
                      ;; sure. The p-song-playfn should also use (now)
    ...
    (sort
     (mapcar #'(lambda (x)
                 ...
                 (let* ((lsample (getf (cdr x) :lsample))
                        (keynum (+ (of-incudine-dsps:lsample-keynum lsample) (getf (cdr x) :transp)))
                        (args (cdr x)))
                   (remf args :transp)
                   ...
                   (apply #'make-instance 'cm::poolevt
                          ...
                          args)))
             (funcall (p-song-playfn p-song)
                      (funcall (p-song-durfn p-song))
                      :amp amp
