...
;;; sennholz@me.comralph.abelein@hfmdk-frankfurt.de
;;;
;;;
;;;                   Song Definitions
;;;
...
(defparameter hfmdk-logo-01
  (let ((dtime 0.05))
    (make-song
     :name "hfmdk-logo-01"
     :durfn (lambda () (r-exp 1.0 2 ))
     :afterfn (lambda () 0)
     :beforefn (lambda () 0)
     :playfn
     (lambda (dur)
       (let* ((numstream (new cycle :of '(7 15 11 13)))
              (now (now))
              (end (+ now dur))
              (args (list :amp -20 :wwidth 0.3 :transp (+ 0 (random 20)))))
         (labels ((recurse-fn (time)
                    (format t "time: ~a, dur: ~a end: ~a~%" time dur end)
                    (if (< time end)
                        (let* ((pattern-dtime (+ 0 (* dtime (random 15))))
                               (next (+ time pattern-dtime))
                               (pattern-dur (+ 4 (random 2.0))))
                          (multiple-value-bind (pstream rstream) (get-streams (next numstream) *pool31-idxs*)
                            (apply #'playpattern pstream rstream time pattern-dur dtime
                                   (append (list :outfn (fig5-out (random 5) *scircle-cw* :scale 2)) args)))
                          (if (< next end) (at next #'recurse-fn next))))))
           (recurse-fn now)))))))

(get-streams 14 *pool31-idxs*)

(defparameter hfmdk-01
  (make-song
   :name "hfmdk-01"
   :playfn
   (playfn 0
           (list
            :g1 *pool31*
            :outfn (fig5-out (random 5) *scircle-cw* :scale 2)
...
(defparameter hfmdk-01
  (make-song
   ...
   (playfn 0
           (list
            ;;            :g1 (subseq *pool31* 2 30)
            :g1 *pool31*
;;;            :bufferfn (lambda (x) x (elt *pool31* 0))
            :outfn (lambda (x) x (values 0 1 (random 1.0)))
            :transpfn (lambda (x) (n-lin (interp x 0 0 1 1) 0 0))
            :ampfn (lambda (x) (n-lin (interp x 0 0 1 1) -15 0))
            :dtimefn (lambda (x) (n-exp (interp x 0 0 1 1) 0.06 0.05))))
   :beforefn (lambda () (r-exp 2 5))
   :durfn (lambda () 0.5)
   :afterfn (lambda () (r-exp 2 5))))
