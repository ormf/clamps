...
*** See also
    ...
    - [[with-proplist/collecting][with-proplist/collecting]]
...
** with-proplist/collecting
   Macro
   #+BEGIN_SRC lisp
     (with-proplist/collecting (keysym valuesym) proplist &body body)
   #+END_SRC
   Like do-proplist but collecting the result.
*** Arguments
    =keysym= -- Symbol bound to all keys of the property list.
...
*** Examples
    #+BEGIN_SRC lisp
      (with-proplist/collecting (key val) '(:a 2 :b 5 :c 4)
        (list key (1+ val)))
      ;; => ((:a 3) (:b 6) (:c 5))
    #+END_SRC
*** See also
