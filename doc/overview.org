...
** What is Clamps?
  ...
  in the hope it might be useful for others, but -like the beauty of
  Art in its own right- without any explict or implicit warranty that
  it fits any purpose whatsoever.
...
#+LATEX: Acknowledgements
  ...
  David Psenicka, CLOG by Rabbibotton, ATS by Juan Pampin and of
  ...
  using the results of their work and benefit from them throughout my
  artistic career. In that sense making Clamps available to the public
  is my contribution intended to pay back to the community.
...
*** Common Music
    ...
    the last Common Lisp version 2.12, released around 2007. Although
    this version already contained realtime capablilities, this
    functionality has been adapted in Clamps using [[Incudine][Incudine]]. Clamps
    extends the functionality of CM by adding classes for [[https://en.wikipedia.org/wiki/SVG][SVG]]
    import/export as well as [[https://en.wikipedia.org/wiki/SFZ_(file_format)][SFZ]] and sample based in-/output realized
    using [[Incudine][Incudine]].
*** FOMUS
...
*** CLOG
    [[https://github.com/rabbibotton/clog][CLOG]] is a web based system written in Common Lisp using Javascript
    in a Webbrowser for rendering Graphical User Interfaces using
    [[https://en.wikipedia.org/wiki/WebSocket][Websockets]] for the communication between Common Lisp and the
    Browser. CLOG closes the gap of missing GUIs in Common Lisp based
    systems due to the time of the definition of the Common Lisp
    Standard in 1990 predating the predominant use of GUIS in current
    computer applications. The most common GUI widgets like sliders,
    number boxes, buttons, interactive graphic displays and VU meters
    have been added to the Clamps system to facilitate the creation of
    sophisticated GUIs nowadays taken for granted in computer music
    environments.
...
*** CLOG extensions
    - /clog-dsp-widgets/
      ...
      Radiobuttons, VU-Meters, Scopes, etc. in a browser including an
      infrastructure for connecting Common Lisp with the GUI parts
      written in JavaScript for client side rendering.
