...
** clog-midi-controller
   clog-midi-controller define the GUI parts of midi controllers
...
** General
   ...
   events[fn:-1]. It
   uses multimethods (also called /multiple dispatch/), dispatching
   different event classes to different output backends.

   The event classes defined by CM 2.12 for output were mainly MIDI
   events of all types and OSC events for SuperCollider.

   Clamps extends these event classes by the following event classes:
   
   - [[dict:sfz][sfz]] (see [[cm-sfz]])
   - [[dict:poolevt][poolevt]]

** cm-sfz
   cm-sfz implements the [[dict:sfz][sfz]] class for Common Music derived from its
   /event/ class. Like any other event class in CM an instance is
   created with the [[dict:new][new]] macro.
   #+BEGIN_SRC lisp
     (new sfz)
   ...
   #+END_SRC
** cm-fomus
   cm-fomus is a [[https://fomus.common-lisp.dev/][FOMUS]] backend for Common Music written by Rick
   Taube. Clamps mainly supports the LilyPond backend of FOMUS.



   For an explanation of the implementation details see the
   [[../fomus/index.html][fomus]] documentation included on ths website.
** cm-svg
   TBA
** cm-svg.rts
   TBA
...
** cl-refs
   ...
   certain situations, like reacting to external events, etc.[fn:7]
...
**** watch
     ...
     with the change of ref-objects[fn:9]. Since actions can also
     ...
     variables and associated actions[fn:10].
...
** cl-sfz
   ...
   amplitude of the recording and other playback information[fn:11].
...
*** Playing sfz events
    ...
    The function [[dict:play-sfz][play-sfz]][fn:12] plays a sound from a sfz preset. It takes
...
[fn:-1] events are instances of objects derived from the /event/ class.

[fn:7] With the notable exception of a value bound by /defconstant/,
as a constant is an /immutable/ value, which never changes.

[fn:8] The implementation ideas are similar to javascript frameworks
...
[fn:9] In Lisp parlance this is the classic example of a
/side-effect/.

[fn:10] For people used to patcher based systems like [[https://en.wikipedia.org/wiki/Pure_Data][Pure Data]] or
...
[fn:11] For an example see the file /"000_​Flute​_nv.sfz"/ located in
the /extra/snd/sfz/Flute-nv// folder of the clamps repository.

[fn:12](play-sfz 60 0 20 :preset :yamaha-grand-piano :pan 0) ;; => nil

[fn:13] For examples see Dan Graham's famous book "On Lisp" or Doug
