...
** cm-poolevt
   ...
   playback. In addition, a poolevt uses a /dy/ slot to enable a
   pitch-linear glissando of the sample from start to end. A poolevt
   also uses lsamples, ignoring their loop points. The sample to be
   played doesn't need to have a defined pitch. Nevertheless, it uses
   a keynum slot for SVG export. The transposition is determined by
   comparing the keynum of a poolevt instance with the keynum of its
   lsample[fn:11].
...
** incudine-bufs
   incudine-bufs implements a registry for buffers. Buffers get added
   with the command [[dict:add-buffer][add-buffer]]. The command takes a relative or
   ...
   [[dict:#sfile-path][*​sfile-path​*]]. The user can push pathnames to that variable.

   If a buffer of that name has already been loaded, add-buffer simply
   returns that buffer. Otherwise it loads the file into an
   incudine:buffer struct, assigns an unused integer /id/ to it
   buffer, updates the registry and returns the loaded buffer:
...
**** watch
     ...
     with the change of ref-objects[fn:14]. Since actions can also
